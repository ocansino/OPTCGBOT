...
class OpponentIntakeTests(unittest.TestCase):
    ...
    def test_guided_attack_logs_attack_event(self) -> None:
        ...
        self.assertEqual(session["events"][-1]["stage"], "attack")
        self.assertIn("Attack declared", session["events"][-1]["summary"])
        self.assertEqual(session["events"][-1]["details"]["action"]["type"], "attack")
    ...
    def test_handle_shorthand_report_can_attack_leader(self) -> None:
        ...
        self.assertTrue(handled)
        self.assertEqual(state["cli_context"]["opponent_turn_history"][-1]["events"][-1]["stage"], "attack")
        self.assertIn("Shorthand attack report", state["cli_context"]["opponent_turn_history"][-1]["events"][-1]["summary"])
    ...
    def test_handle_shorthand_report_can_chain_attack_and_counter_same_target(self) -> None:
        ...
        handled = cli_game.handle_shorthand_report(self.engine, state, "attack OP12-119 leader then counter OP12-098 same target")
    ...
    def test_handle_shorthand_report_can_use_same_target_when_last_target_was_leader(self) -> None:
        ...
        handled_attack = cli_game.handle_shorthand_report(self.engine, state, "attack OP12-119 leader")
